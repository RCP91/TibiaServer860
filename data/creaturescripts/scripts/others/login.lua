...
function onLogin(player)
	...
	-->player:setStorageValue(Storage.NOME_QUEST_NO_ARQUIVO_051-storages.lua.ID OU NOME REFERENTE AO START DA QUEST EM QUESTS.XML, ID REFERENTE AO START OU O END DA QUEST)
		...
		player:setStorageValue(Storage.WhiteRavenMonasteryQuest.Passage, 1)
		player:setStorageValue(Storage.WhiteRavenMonasteryQuest.Passage, 1)
		player:setStorageValue(Storage.TheNewFrontier.Mission04, 1)
		player:setStorageValue(Storage.TheNewFrontier.Mission01, 1)
		player:setStorageValue(Storage.TheNewFrontier.Mission02, 1)
				player:setStorageValue(Storage.TheNewFrontier.Mission03, 1)
				player:setStorageValue(Storage.TheNewFrontier.Mission05, 1)
				player:setStorageValue(Storage.TheNewFrontier.Mission06, 1)
				player:setStorageValue(Storage.TheNewFrontier.Mission07, 1)
		player:setStorageValue(Storage.TheNewFrontier.Mission08, 1)				
				player:setStorageValue(Storage.TheNewFrontier.Mission09, 1)
...
            - Commands -]]
 	...
 	-- OPEN CHANNELS
	if table.contains({"Kazordoon", "Gray Beach", "Feyrist", "Krailos", "Rathleton", "Ab'Dendriel", "Liberty Bay", "Farmine", "Darashia", "Ankrahmun", "Rookgaard", "Dawnport", "thais", "carlin", "venore", "issavi", "yalahar", "edron", "port hope"}, player:getTown():getName())then
		player:openChannel(3) -- world chat
		player:openChannel(6) -- advertsing rook main
		player:openChannel(7) -- world chat
   ...
   -- player:sendTextMessage(messageType or MESSAGE_STATUS_CONSOLE_ORANGE, 'Bem vindo ao Order-Global!')
	...
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_ORANGE, 'Comandos: !bless, !aol, !shovel, !pick, !rope, !machete, !xp')
