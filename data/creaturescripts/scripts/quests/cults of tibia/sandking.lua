	...
	elseif str == 4 then
		...
		if ((maxhealth*0.50) > creature:getHealth() ) then
			...
			for _, pos in pairs(ps) do
				...
				monster:getHealth(monster:getMaxHealth()/2)
				if pass == 0 then
		...
		-- addEvent(function()
			...
			-- for _x= frompos.x, topos.x, 1 do
				-- for _y= frompos.y, topos.y, 1 do
					-- for _z= frompos.z, topos.z, 1 do
						-- local tile = Tile(Position(_x, _y, _z))
						-- if tile and tile:getTopCreature() and tile:getTopCreature():isMonster() and tile:getTopCreature():getName() == "The Sandking" then
							-- spawn = false
							-- break
						-- end
					-- end
			...
			-- if not spawn then
				-- return true
			-- end
			-- Game.setStorageValue("sandking", 5)
		-- end, 2000)
	end
	return true
end
