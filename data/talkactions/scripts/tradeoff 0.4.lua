...
function onSay(cid, words, param, channel)
	...
	if (p[1]) then
		...
		if (p[1] == "add") then
			...
			if hasSubType(item.id).charges > 0 then
				charges = item.type
				count = 1
			else
				charges = "DEFAULT"
				...
				if isNumber(p[2]) then
					...
					if tradeItemID then
						...
						elseif (tradeItemID == item.id) then
							...
							return true						
						elseif hasSubType(tradeItemID).corpseType > 0 then
							doPlayerSendTextMessage(cid, config.errorMsgType, "You can not buy a corpse.")
							return true
						elseif not hasSubType(tradeItemID).pickupable then
						...
						if getPlayerGUID(cid) ~= owner then
							...
							if isTrade > 0 then
								...
								if not (getPlayerItemCount(cid, cost) >= costCount) then
									...
									return true
								elseif getItemDefaultDuration(cost) > 0 or hasSubType(cost).charges > 0 then
									local item = getPlayerSlotItem(cid, CONST_SLOT_AMMO)
									if (item.uid > 0 and item.id == cost) then
										...
										elseif hasSubType(cost).charges > 0 then
											ogCostCount = costCount
											costCount = item.type
											if item.type < hasSubType(cost).charges then
												result.free(queryResult)
												doPlayerSendTextMessage(cid, config.errorMsgType, "The "..itemCostName.." needs to be brand new.")
												return true
											end
...
function getItemDefaultDuration(itemID)
	if hasSubType(itemID).decayTime <= 0 then
		if hasSubType(itemID).transformUseTo > 0 then
			return hasSubType(hasSubType(itemID).transformUseTo).decayTime
		elseif hasSubType(itemID).transformEquipTo > 0 then
			return hasSubType(hasSubType(itemID).transformEquipTo).decayTime
		end
	else
		return hasSubType(itemID).decayTime
	end
	return 0
end
