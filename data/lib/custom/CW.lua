local config = {
	...
	timeToKill = 60, -- Tempo em segundos para matar todos os geradores. (A diferença entre o tempo da morte de todos deve ser no maximo 60 segundos)
	generators = {
	...
	teleport = {pos = Position(31884, 31874, 4), topos = Position(31885, 31874, 4), duration = 60}, -- Teleporte gerado apos morte dos geradores para terceira sala. ultima sala baixo
}
...
function CASTLE:removeTeleport()
	self:spawnGenerator()
	local teleport = Tile(config.teleport.pos):getItemById(1387)
	if teleport then
		teleport:remove()
	end
...
function CASTLE:createTeleport()
	local teleport = Game.createItem(1387, 1, config.teleport.pos)
	if teleport then
