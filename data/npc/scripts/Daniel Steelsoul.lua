...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") then
		if getPlayerStorageValue(cid, Storage.TibiaTales.AgainstTheSpiderCult) < 1 then
		...
		elseif getPlayerStorageValue(cid, Storage.TibiaTales.AgainstTheSpiderCult) == 5 then
			setPlayerStorageValue(cid, Storage.TibiaTales.AgainstTheSpiderCult, 6)
			talkState[talkUser] = 0
			doPlayerAddItem(cid, 7887, 1)
			selfSay("What? YOU DID IT?!?! That's...that's...er....<drops a piece of paper. You see the headline 'death certificate'> like I expected!! Here is your reward.", cid)
		end
	elseif msgcontains(msg, "yes") then
		if talkState[talkUser] == 1 then
			setPlayerStorageValue(cid, Storage.TibiaTales.DefaultStart, 1)
			setPlayerStorageValue(cid, Storage.TibiaTales.AgainstTheSpiderCult, 1)
			selfSay({
