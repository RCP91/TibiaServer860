...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'commander') then
		return selfSay('I\'m responsible for the security and reward heroes to our cause. If you are looking for missions, talk to Gnomilly, Gnombold and Gnomagery.', cid)
	end

	if msgcontains(msg, 'reward') then
		return selfSay('I can sell special outfit parts. If your fame is high enough, you might be {worthy} of such a reward.', cid)
	end

	if msgcontains(msg, 'spike') then
		return selfSay(speech, cid)
	end

	if msgcontains(msg, 'worthy') then
		if player:getFamePoints() < 100 then
			return selfSay('You are not worthy of a special reward yet.', cid)
		end

		talkState[cid] = 'worthy'
		return selfSay('You can acquire the {basic} outfit for 1000 Gold, the {first} addon for 2000 gold and the {second} addon for 3000 gold. Which do you want to buy?', cid)
	end
	...
	if talkState[cid] == 'worthy' then
		if msgcontains(msg, 'basic') then
			if getPlayerLevel(cid) < 25 then
				talkState[cid] = nil
				return selfSay('You do not have enough level yet.', cid)
			end

			if player:hasOutfit(player:getSex() == 0 and 575 or 574) then
				talkState[cid] = nil
				return selfSay('You already have that outfit.', cid)
			end

			talkState[cid] = 'basic'
			return selfSay('Do you want to buy the basic outfit for 1000 Gold?', cid)
		elseif msgcontains(msg, 'first') then
			...
			if not player:hasOutfit(player:getSex() == 0 and 575 or 574) then
				talkState[cid] = nil
				return selfSay('You do not have the Cave Explorer outfit.', cid)
			end

			if player:hasOutfit(player:getSex() == 0 and 575 or 574, 1) then
				talkState[cid] = nil
				return selfSay('You already have that addon.', cid)
			end

			talkState[cid] = 'first'
			return selfSay('Do you want to buy the first addon for 2000 Gold?', cid)
		elseif msgcontains(msg, 'second') then
			...
			return selfSay('Do you want to buy the second addon for 3000 Gold?', cid)
		end
	end
	...
	if talkState[cid] == 'basic' then
		if msgcontains(msg, 'yes') then
			if not player:removeMoney(1000) then
				talkState[cid] = nil
				return selfSay('You do not have that money.', cid)
			end
		end
		player:removeFamePoints(100)
		player:addOutfit(player:getSex() == 0 and 575 or 574)
		talkState[cid] = nil
		return selfSay('Here it is.', cid)
	elseif talkState[cid] == 'first' then
		...
		player:removeFamePoints(100)
		doPlayerAddOutfit(cid, player:getSex() == 0 and 575 or 574, 1)
		talkState[cid] = nil
		return selfSay('Here it is.', cid)
	elseif talkState[cid] == 'second' then
		...
		player:removeFamePoints(100)
		doPlayerAddOutfit(cid, player:getSex() == 0 and 575 or 574, 2)
		talkState[cid] = nil
		return selfSay('Here it is.', cid)
	end
	return true
