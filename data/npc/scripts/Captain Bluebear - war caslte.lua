...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "info") or msgcontains(msg, "help") then
		npcHandler:say(("You can teleport to the Castle, talking {castle war}. %s"):format(CASTLE:getDominantGuild() and CASTLE:getDominantGuild():getName() .. "dominates the castle today." or "no guild currently dominates the castle."), cid)
		elseif msgcontains(msg, "castle war") then
		local dominant = CASTLE:getDominantGuild()
		local guild = player:getGuild()
		if dominant and guild and dominant == guild then
			CASTLE:teleportPlayer(player)
			return true
		end
		npcHandler:say(("You are sure you want to invade the castle? %s"):format(dominant and "The guild dominant will be alerted." or ""), cid)
		npcHandler.topic[cid] = 1
		elseif (msgcontains(msg, "yes") or msgcontains(msg, "sim")) and npcHandler.topic[cid] == 1 then
		local retmsg = CASTLE:teleportPlayer(player)
		npcHandler:say(retmsg, cid)
