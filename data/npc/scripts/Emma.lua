...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'yes') then
		if talkState[talkUser] == 1 then
			...
			selfSay('I am still a bit sceptical, but well, welcome to the girls brigade.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 2 then
			if doPlayerRemoveItem(cid, 8190, 1) then
				setPlayerStorageValue(cid, Storage.secretService.CGBMission01, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 3)
				selfSay('How unnecessarily complicated, but that\'s the way those Thaians are. In the end we got what we wanted and they can\'t do anything about it.', cid)
			else
				selfSay('Bring me the spellbook.', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 3 then
			if doPlayerRemoveItem(cid, 7736, 1) then
				setPlayerStorageValue(cid, Storage.secretService.CGBMission02, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 5)
				selfSay('I think the druids will be pleased to hear that the immediate threat has been averted.', cid)
			else
				selfSay('Bring me the heart as proof.', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 4 then
			setPlayerStorageValue(cid, Storage.secretService.CGBMission03, 3)
			setPlayerStorageValue(cid, Storage.secretService.Quest, 7)
			selfSay('Great! This blow strikes them where it hurts most: profit.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 5 then
			if doPlayerRemoveItem(cid, 7702, 1) then
				setPlayerStorageValue(cid, Storage.secretService.CGBMission04, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 9)
				selfSay('I hope our craftsmen can do something with this stuff. For me it makes hardly any sense.', cid)
			else
				selfSay('You need to bring me those plans!', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 6 then
			if doPlayerRemoveItem(cid, 7703, 1) then
				setPlayerStorageValue(cid, Storage.secretService.CGBMission05, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 11)
				selfSay('I will have this correspondence examined by our specialists. I am sure they are quite revealing.', cid)
			else
				selfSay('Bring me back some hints or something!', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 7 then
			if doPlayerRemoveItem(cid, 7704, 1) then
				setPlayerStorageValue(cid, Storage.secretService.CGBMission06, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 13)
				selfSay('I hope this old book will do those researches any good. Personally I see little use to proof some bloodlines after we cut all ties to Thais.', cid)
			else
				selfSay('You need to bring us that book of family trees!', cid)
			end
			talkState[talkUser] = 0
	...
	elseif msgcontains(msg, 'mission') then
		if getPlayerStorageValue(cid, Storage.secretService.Quest) == 1 and getPlayerStorageValue(cid, Storage.secretService.TBIMission01) < 1 and getPlayerStorageValue(cid, Storage.secretService.CGBMission01) < 1 then
			...
			setPlayerStorageValue(cid, Storage.secretService.CGBMission01, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission01) == 1 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 2
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission01) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 3 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 4)
			setPlayerStorageValue(cid, Storage.secretService.CGBMission02, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission02) == 1 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 3
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission02) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 5 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 6)
			setPlayerStorageValue(cid, Storage.secretService.CGBMission03, 1)
			doPlayerAddItem(cid, 7698, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission03) == 2 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 4
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission03) == 3 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 7 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 8)
			setPlayerStorageValue(cid, Storage.secretService.CGBMission04, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission04) == 1 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 5
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission04) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 9 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 10)
			setPlayerStorageValue(cid, Storage.secretService.CGBMission05, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission05) == 1 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 6
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission05) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 11 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 12)
			setPlayerStorageValue(cid, Storage.secretService.CGBMission06, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission06) == 1 then
			selfSay('Have you been successful?', cid)
			talkState[talkUser] = 7
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission06) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 13 then
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.CGBMission06) == 2 and getPlayerStorageValue(cid, Storage.secretService.Mission07) == 1 then
			selfSay('Have you been successful?', cid)
