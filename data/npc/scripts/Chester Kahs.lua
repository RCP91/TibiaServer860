...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') then
		...
		elseif talkState[talkUser] == 3 then
			if doPlayerRemoveItem(cid, 2177, 1) then
				selfSay('Thank you! Take this ring. If you ever need a healing, come, bring the scroll, and ask me to {heal}.', cid)
				doPlayerAddItem(cid, 2168, 1)
			else
				selfSay('Sorry, but you have none.', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 4 then
			if doPlayerRemoveItem(cid, 2168, 1) then
				selfSay('So be healed!', cid)
				player:addHealth(player:getMaxHealth())
				Npc():getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
			else
				selfSay('Sorry, you are not worthy!', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 5 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 1)
			selfSay('Then I welcome you to the TBI. This is a great moment for you, remember it well. Talk to me about your missions whenever you feel ready.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 6 then
				setPlayerStorageValue(cid, Storage.secretService.TBIMission01, 3)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 3)
				selfSay('I think they understood the warning the way it was meant. If not, you will have to visit Venore soon again. But for now it\'s settled.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 7 then
			if doPlayerRemoveItem(cid, 7696, 1) then
				setPlayerStorageValue(cid, Storage.secretService.TBIMission02, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 5)
				selfSay('Thank you, we can finally let them have some closure regarding this.', cid)
			else
				selfSay('Please bring me some proof of his whereabouts.', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 8 then
			if doPlayerRemoveItem(cid, 14324, 1) then
				setPlayerStorageValue(cid, Storage.secretService.TBIMission03, 3)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 7)
				selfSay('I can only hope that this information are as valuable as we expected it. A good man died for them.', cid)
			else
				selfSay('Please bring me some valuable information!', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 9 then
			if doPlayerRemoveItem(cid, 14325, 1) then
				setPlayerStorageValue(cid, Storage.secretService.TBIMission04, 2)
				setPlayerStorageValue(cid, Storage.secretService.Quest, 9)
				selfSay('Ah yes, very interesting. Almost as I suspected. It\'s a good thing that we got those documents in our hands.', cid)
			else
				selfSay('We need those intelligence reports, do whatever you need to do agent!', cid)
			end
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 10 then
			setPlayerStorageValue(cid, Storage.secretService.TBIMission05, 3)
			setPlayerStorageValue(cid, Storage.secretService.Quest, 11)
			selfSay('Now that Venore is of nearly no importance anymore, there is only Carlin left to deal with.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 11 then
			setPlayerStorageValue(cid, Storage.secretService.TBIMission06, 3)
			setPlayerStorageValue(cid, Storage.secretService.Quest, 13)
			selfSay('I already heard that our little trick worked quite well. Several officials of Carlin are already on their way to repair the damage done to their diplomatic efforts. It will not only cost them much money but also quite some time.', cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 12 then
			if doPlayerRemoveMoney(cid, 1000) then
				doPlayerAddItem(cid, 7700, 1)
				selfSay('Here you are. Better don\'t loose it again.', cid)
			else
		...
		elseif talkState[talkUser] == 13 then
			if doPlayerRemoveItem(cid, 7699, 1) then
				...
				doPlayerAddItem(cid, 7960, 1)
				selfSay('You have done superb work agent, I grant you the title of Top Agent! Here\'s a little gift you might find useful.', cid)
			else
	...
	elseif msgcontains(msg, 'no') then
		if talkState[talkUser] == 1 then
			selfSay('Then don\'t bother me with it. I\'m a busy man.', cid)
		elseif talkState[talkUser] == 3 then
			selfSay('Traitor!', cid)
	...
	elseif talkState[talkUser] == 2 then
		if msgcontains(msg, 'magic') and msgcontains(msg, 'crystal') and msgcontains(msg, 'lugri') and msgcontains(msg, 'deathcurse') then
			selfSay('That\'s terrible! Will you give me the crystal?', cid)
			talkState[talkUser] = 3
		else
			selfSay('Tell me precisely what he asked you to do! It\'s important!', cid)
		end
	elseif msgcontains(msg, 'heal') then
		selfSay('Do you need the healing now?', cid)
	...
	elseif msgcontains(msg, 'mission') then
		if getPlayerStorageValue(cid, Storage.secretService.Quest) == 1 and getPlayerStorageValue(cid, Storage.secretService.AVINMission01) < 1 and getPlayerStorageValue(cid, Storage.secretService.CGBMission01) < 1 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 2)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission01, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission01) == 2 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 6
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission01) == 3 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 3 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 4)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission02, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission02) == 1 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 7
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission02) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 5 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 6)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission03, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission03) == 2 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 8
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission03) == 3 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 7 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 8)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission04, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission04) == 1 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 9
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission04) == 2 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 9 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 10)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission05, 1)
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission05) == 2 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 10
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission05) == 3 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 11 then
			setPlayerStorageValue(cid, Storage.secretService.Quest, 12)
			setPlayerStorageValue(cid, Storage.secretService.TBIMission06, 1)
			doPlayerAddItem(cid, 7700, 1)
			selfSay({
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission06) == 2 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 11
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission06) == 3 and getPlayerStorageValue(cid, Storage.secretService.Quest) == 13 then
			...
			talkState[talkUser] = 0
		elseif getPlayerStorageValue(cid, Storage.secretService.TBIMission06) == 3 and getPlayerStorageValue(cid, Storage.secretService.Mission07) == 1 then
			selfSay('Have you fulfilled your current mission?', cid)
			talkState[talkUser] = 13
		end
	elseif msgcontains(msg, 'disguise') then
		if getPlayerStorageValue(cid, Storage.secretService.TBIMission06) == 1 then
			selfSay('If you lost or wasted your disguise kit I can replace it. It will cost you 1000 gold though since you lost royal property. Is that ok for you?', cid)
