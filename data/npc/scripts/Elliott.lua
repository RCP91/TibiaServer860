...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "abandoned sewers")) then
		...
		elseif(getPlayerStorageValue(cid, 20062) == 21) then
            ...
            setPlayerStorageValue(cid, Storage.Oramond.DoorAbandonedSewer, 1)
		elseif(getPlayerStorageValue(cid, 10050) == 5) then
		    selfSay("I'm glad to see you back alive and healthy. Did you find anything interesting that you want to {report}?", cid)
			talkState[talkUser] = 7
	end
	elseif(msgcontains(msg, "mission")) then
		if(talkState[talkUser] == 0) then
			selfSay("The sewers need repair. You in?", cid)
			talkState[talkUser] = 2
		elseif(getPlayerStorageValue(cid, 10050) >= 3) then
			selfSay("Elliott's keeps calling it that. It's just another job! You fixed some broken pipes and stuff? Let me check, {ok}?", cid)
			talkState[talkUser] = 3
		end
	elseif(msgcontains(msg, "yes")) then
		if(talkState[talkUser] == 2) then
			selfSay("Good. Broken pipe and generator pieces, there's smoke evading. That's how you recognise them. See how you can fix them using your hands. Need about, oh, twenty of them at least repaired. Report to me or Jacob", cid)
	...
	elseif(msgcontains(msg, "ok")) then
		if(talkState[talkUser] == 3) then
			selfSay("Good. Thanks, man. That's one vote you got for helping us with this.", cid)
			talkState[talkUser] = 0
	...
	if(getPlayerStorageValue(cid, 10050) == 5) then
		--if(talkState[talkUser] == 7) then
			...
			else selfSay("You already reported this mission, go to the next.", cid)
		end
	end
