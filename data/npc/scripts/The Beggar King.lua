...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission")) then
		if(getPlayerStorageValue(cid, 10050) <= 2 and getPlayerStorageValue(cid, 871241) == 1) then
			selfSay("So I guess you are the one that the magistrate is sending to look after us, eh? ", cid)
			talkState[talkUser] = 1
			else
			selfSay("You need some quests then come and talk with me again.", cid)
		end
	...
	elseif(msgcontains(msg, "something")) then
		if(talkState[talkUser] == 2) then
			selfSay("Yeah. No one has seen it and lived to tell the tale. People are missing and sometimes there are {traces} of blood or someone heard a scream, but that's all. We have no idea if the killer is a man or a beast, but there is something out there", cid)
			talkState[talkUser] = 3
		end
	elseif(msgcontains(msg, "traces")) then
		if(talkState[talkUser] == 3) then
			selfSay("Some of the more daring of us tried to follow the tracks that were left, but they always lost the trail close to the abandoned sewers, in the east of the sewer system.", cid)
