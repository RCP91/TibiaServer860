...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "tome") or msgcontains(msg, "knowledge") then
		...
		if getPlayerStorageValue(cid, Storage.TheNewFrontier.Mission08) == 2 then
			if getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) < 1 then --tome1
				selfSay("Oh! That sounds fascinating. Have you found a Tome of Knowledge for me to read?", cid)
				talkState[talkUser] = 1
			elseif getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 1 and getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) <= 5 then --tome2 - tome6
				selfSay("Oh! That sounds fascinating. Have you found a new Tome of Knowledge for me to read?", cid)
				talkState[talkUser] = getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge)+1
			elseif getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 6 and getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) <= 11 then --tome7 - tome12
				--The New Frontier Quest completed to trade more Tomes of Knowledge with NPC Cael.
				if getPlayerStorageValue(cid, Storage.TheNewFrontier.Mission10) == 1 then
					selfSay("Oh! That sounds fascinating. Have you found a new Tome of Knowledge for me to read?", cid)
					talkState[talkUser] = getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge)+1
				else
					selfSay("I'm sorry I'm busy. Speak with Ongulf to get some missions!", cid)
				end
			elseif getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 12 then -- more then 12 tomes
				selfSay("Oh! That sounds fascinating. Have you found a Tome of Knowledge for me to read? I have the feeling though that I can only share some of my experience with you now. Is that alright with you?", cid)
				talkState[talkUser] = 13
			end
		else
			selfSay("I'm sorry I'm busy. Speak with Ongulf to get some missions!", cid)
		end
	elseif msgcontains(msg, "yes") and talkState[talkUser] >= 1 and talkState[talkUser] <= 13 then
		if doPlayerRemoveItem(cid, 11134, 1) then --remove tome
			if talkState[talkUser] == 1 then	--tome1
				selfSay("Thank you! I look forward to reading this interesting discovery of yours and learn a few things about {Zao}.", cid)
				setPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge, 1)
				talkState[talkUser] = 21
			elseif talkState[talkUser] >= 2 and talkState[talkUser] <= 12 then --tome2 - tome12
				selfSay("Thank you! I look forward to reading this interesting discovery of yours and learn a few things about {Zao}.", cid)
				setPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge, getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) + 1)
				talkState[talkUser] = getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge)+20
			elseif talkState[talkUser] == 13 then -- more then 12 tomes
				player:addExperience(5000, true)
				selfSay("Thank you! I look forward to reading this interesting discovery of yours and learn a few things about {Zao}. Let me share some experience with you.", cid)
				talkState[talkUser] = 33
			end
		else
			selfSay("You dont have one!", cid)
			talkState[talkUser] = 0
		end
	elseif(msgcontains(msg, "zao")) then
		if talkState[talkUser] == 21 then	--tome1
			selfSay("I've learnt more about the {lizard} culture. It's really fascinating.", cid)
			talkState[talkUser] = 40
		elseif talkState[talkUser] == 22 then	--tome2
			selfSay("I've learnt more about the {minotaur} culture. It's really fascinating.", cid)
			talkState[talkUser] = 41
		elseif talkState[talkUser] == 23 then	--tome3
			selfSay("I've learnt more about the {Draken} culture by now. It's really fascinating.", cid)
			talkState[talkUser] = 42
		elseif talkState[talkUser] == 24 then	--tome4
			selfSay("I've learnt something interesting about a certain {food} that the lizardmen apparently prepare.", cid)
			talkState[talkUser] = 43
		elseif talkState[talkUser] == 25 then	--tome5
			selfSay("I've learnt something interesting about a city called {Zzaion}.", cid)
			talkState[talkUser] = 44
		elseif talkState[talkUser] == 26 then	--tome6
			selfSay("I've learnt a few things about the primitive {human} culture on this continent.", cid)
			talkState[talkUser] = 45
		elseif talkState[talkUser] == 27 then	--tome7
			selfSay("I've learnt something interesting about the Zao {steppe}.", cid)
			talkState[talkUser] = 46
		elseif talkState[talkUser] == 28 then	--tome8
			selfSay("I've learnt a few things about an illness, or how I prefer to call it, {corruption} of this land.", cid)
			talkState[talkUser] = 47
		elseif talkState[talkUser] == 29 then	--tome9
			selfSay("I've learnt something interesting about the Draken {origin}.", cid)
			talkState[talkUser] = 48
		elseif talkState[talkUser] == 30 then	--tome10
			selfSay("This book actually IS about Zao. Not about the continent, but about the mythical {founder} of the lizard dynasty.", cid)
			talkState[talkUser] = 49
		elseif talkState[talkUser] == 31 then	--tome11
			selfSay("I've learnt something interesting about {dragons} and their symbolism.", cid)
			talkState[talkUser] = 50
		elseif talkState[talkUser] == 32 then	--tome12
			selfSay("The last tome contained a lot of information about status symbols and insignia - such as {thrones} - and reveals some of the power structures in Zao.", cid)
			talkState[talkUser] = 51
		elseif talkState[talkUser] == 33 then	--more than tome12
			selfSay("I've learnt many things from your books. Still, I guess that's just a fragment of what I could still discover about this interesting continent.", cid)
			talkState[talkUser] = 0
		end
	elseif(msgcontains(msg, "lizard")) then --tome1
		if talkState[talkUser] == 40 then
			selfSay({
	...
	elseif msgcontains(msg, "lantern") then
		if getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 11 then
		 	selfSay("Have you brought me a red lantern for a dragon statue?", cid)
			talkState[talkUser] = 65
		end
	elseif msgcontains(msg, "yes") and talkState[talkUser] == 65 then
		if doPlayerRemoveItem(cid, 11206, 1) then
			doPlayerAddItem(cid, 11133,1)
			selfSay("Let's put this little lantern here.. there you go. I wrap it up for you, just unwrap it in your house again!", cid)
			talkState[talkUser] = 0
		else
			selfSay("You don't have a red lantern.", cid)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, "cloth") then
		if getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 12 then
		 	selfSay("Have you brought me a piece of red cloth? I can make that throne for you if you want. But remember, I won't do that all the time - so try and don't destroy it, okay?", cid)
			talkState[talkUser] = 66
		end
	elseif msgcontains(msg, "yes") and talkState[talkUser] == 66 then
		if doPlayerRemoveItem(cid, 5911, 1) then
			doPlayerAddItem(cid, 11205,1)
			selfSay("Let's put this cloth over the seat.. there you go. I wrap it up for you, just unwrap it in your house again!", cid)
			talkState[talkUser] = 0
		else
			selfSay("You don't have a red cloth.", cid)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, "crest") then
		if player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 336 or 335) and getPlayerItemCount(cid, 11116) > 0 then
		 	selfSay("Oh, wow! Now THAT is an interesting relic! Can I have that serpent crest?", cid)
			talkState[talkUser] = 60
		elseif player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 336 or 335) and getPlayerItemCount(cid, 11115) > 0 then
			selfSay("Oh, wow! Now THAT is an interesting relic! Can I have that tribal crest?", cid)
			talkState[talkUser] = 61
		else
			selfSay("You don't have a Warmaster Outfit or the crest to get the Addons.", cid)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, "yes") and talkState[talkUser] >= 60 and talkState[talkUser] <= 61 then
		if talkState[talkUser] == 60 then
			if not player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 336 or 335, 1) and doPlayerRemoveItem(cid, 11116, 1) then
				doPlayerAddOutfit(cid, 335, 1)
				doPlayerAddOutfit(cid, 336, 1)
				player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
				selfSay("Thank you! Let me reward you with something I stumbled across recently and which might fit your warmaster outfit perfectly.", cid)
				talkState[talkUser] = 0
			else
				selfSay("You don't have a crest or already have this Outfitaddon.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 61 then
			if not player:hasOutfit(player:getSex() == PLAYERSEX_FEMALE and 336 or 335, 2) and doPlayerRemoveItem(cid, 11115, 1) then
				doPlayerAddOutfit(cid, 335, 2)
				doPlayerAddOutfit(cid, 336, 2)
				setPlayerStorageValue(cid, 48055, 1)   --QuestPoints
				player:getPosition():sendMagicEffect(CONST_ME_MAGIC_BLUE)
				selfSay("Thank you! Let me reward you with something I stumbled across recently and which might fit your warmaster outfit perfectly.", cid)
				talkState[talkUser] = 0
			else
	...
	elseif msgcontains(msg, "trade") then
		if getPlayerStorageValue(cid, Storage.TheNewFrontier.TomeofKnowledge) >= 6 then
			
			local items = setNewTradeTable(getTable(player))
				openShopWindow(cid, getTable(player), onBuy, onSell)
				selfSay("Keep in mind you won't find better offers here. Just browse through my wares.", cid)
		else
			selfSay("Sorry, I don't have Items to trade now.", cid)
		end
	end
	return true
end
