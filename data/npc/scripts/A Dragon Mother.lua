...
local function greetCallback(cid)
	...
	if getPlayerStorageValue(cid, Storage.ForgottenKnowledge.BabyDragon) < 1 then
		npcHandler:setMessage(MESSAGE_GREET, "Greetings humans! Consider yourselfs lucky, I'm in need of {help}.")
		talkState[talkUser] = 1
		return true
	elseif getPlayerStorageValue(cid, Storage.ForgottenKnowledge.AccessMachine) == 1 then
		npcHandler:setMessage(MESSAGE_GREET, 'Grrr.')
		return true
	elseif getPlayerStorageValue(cid, Storage.ForgottenKnowledge.HorrorKilled) >= 1 then
		...
		setPlayerStorageValue(cid, Storage.ForgottenKnowledge.AccessMachine, 1)
	end
	return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "yes") then
		if talkState[talkUser] == 3 then
			...
			setPlayerStorageValue(cid, Storage.ForgottenKnowledge.BabyDragon, 1)
