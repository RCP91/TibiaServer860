...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") or msgcontains(msg, "quest") then
		local value = getPlayerStorageValue(cid, Storage.ElementalSphere.QuestLine)
		if value < 1 then
		...
		elseif value == 1 then
			if getPlayerItemCount(cid, player:isSorcerer() and 8304 or player:isDruid() and 8305 or player:isPaladin() and 8300 or player:isKnight() and 8306) > 0 then
				setPlayerStorageValue(cid, Storage.ElementalSphere.QuestLine, 2)
				selfSay({
				...
				selfSay("You need some kind of pure elemental soil from the " .. (player:isSorcerer() and "Fire" or player:isDruid() and "Earth" or player:isPaladin() and "Ice" or player:isKnight() and "Energy") .. " Overlord. Come back when you've got it.", cid)
			end
			talkState[talkUser] = 0
		elseif value == 2 then
			if doPlayerRemoveItem(cid, 8310, 1) and getPlayerStorageValue(cid, Storage.ElementalSphere.QuestLine) < 3 then
				...
				doPlayerAddItem(cid, player:isSorcerer() and 8867 or player:isDruid() and 8869 or player:isPaladin() and 8853 or player:isKnight() and 8883, 1)
				setPlayerStorageValue(cid, Storage.ElementalSphere.QuestLine, 3)
			end
		end
	elseif talkState[talkUser] == 1 and msgcontains(msg, "yes") then
		setPlayerStorageValue(cid, Storage.ElementalSphere.QuestLine, 1)
