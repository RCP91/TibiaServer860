...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "errand") or msgcontains(msg, "gold") then
		if getPlayerStorageValue(cid, Storage.TheShatteredIsles.TheErrand) == 1 then
			selfSay("Oh, so you brought some gold from Eleonore to me?", cid)
			talkState[talkUser] = 1
		end
	elseif msgcontains(msg, "yes") then
		if talkState[talkUser] == 1 then
			if doPlayerRemoveMoney(cid, 200) then
				selfSay("Hmm, it seems that Eleonore does trust you. Perhaps she is even right. However. Since we need some help right now I guess we can't be too picky. Return to Eleonore and tell her the secret password: 'peg leg'. She will tell you more about her problem.", cid)
				setPlayerStorageValue(cid, Storage.TheShatteredIsles.TheErrand, 2)
