...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "missions") and talkState[talkUser] == 1 then
	-- Checagem para o FINAL BOSS
	if getPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission) > 2 then
		selfSay({"You have already fulfilled your job to my full satisfaction. The cults are investigated and the final boss is eliminated. I have nothing more for you to do. Fare you well!"}, cid)
		talkState[talkUser] = 0
		playerTopic[cid] = 0
		elseif getPlayerStorageValue(cid, Storage.CultsOfTibia.Minotaurs.Mission) == 6 and getPlayerStorageValue(cid, Storage.CultsOfTibia.Life.Mission) == 10 and getPlayerStorageValue(cid, Storage.CultsOfTibia.MotA.Mission) == 15
		and getPlayerStorageValue(cid, Storage.CultsOfTibia.Barkless.Mission) == 7 and getPlayerStorageValue(cid, Storage.CultsOfTibia.Misguided.Mission) == 5 and getPlayerStorageValue(cid, Storage.CultsOfTibia.Orcs.Mission) == 3
		and getPlayerStorageValue(cid, Storage.CultsOfTibia.Humans.Mission) == 3 and getPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission) < 2 then
			selfSay({"It seems to me that you have done all the missions I gave you. All the cults have been revealed and now you can kill their leader, the final boss, to save the world from a possible catastrophe."}, cid)
			talkState[talkUser] = 8
			playerTopic[cid] = 8
			if getPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission) < 1 then
			setPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission, 1)
			end
		elseif getPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission) == 2 then
			selfSay({"You did it! You put an end to the cults, and as a return, here's your reward."}, cid)
			talkState[talkUser] = 9
			playerTopic[cid] = 9
			local item = ""
			if player:getVocation():getId() == 1 or player:getVocation():getId() == 5 then -- MS
			doPlayerAddItem(cid, 29426)
			item = "reflecting crown"
			end
			if player:getVocation():getId() == 2 or player:getVocation():getId() == 6 then -- ED
			doPlayerAddItem(cid, 29423)
			item = "leaf crown"
			end
			if player:getVocation():getId() == 3 or player:getVocation():getId() == 7 then -- RP
			doPlayerAddItem(cid, 29425)
			item = "incandescent crown"
			end
			if player:getVocation():getId() == 4 or player:getVocation():getId() == 8 then -- EK
			doPlayerAddItem(cid, 29424)
			item = "iron crown"
			end
			player:addExperience(50000)
			doPlayerAddItem(cid, 29422)
			setPlayerStorageValue(cid, Storage.CultsOfTibia.finalBoss.Mission, 3)
		...
		selfSay({"In wich of the following topics are you interested in? Cult of {Life}, Cult of {Prosperity}, Cult of the {Minotaurs}, Cult of the {Barkless}, Cult of the {Misguided}, Cult of {Orcs} or Cult of the {Humans}?"}, cid)
	...
	local missionsTable = config.missions[msg:lower()]
		if missionsTable then
			...
			if getPlayerStorageValue(cid, Stg[cid]) > 0 and getPlayerStorageValue(cid, Stg[cid]) == value[cid] then
				selfSay(missionsTable.completeText, cid)
				setPlayerStorageValue(cid, Stg[cid], getPlayerStorageValue(cid, Stg[cid]) + 1)
				...
				talkState[talkUser] = 0
				playerTopic[cid]= 0
			elseif getPlayerStorageValue(cid, Stg[cid]) > 0 and getPlayerStorageValue(cid, Stg[cid]) > value[cid] then
				selfSay({"You already done this mission."}, cid)
				talkState[talkUser] = 2
				playerTopic[cid]= 2
			else
				selfSay(missionsTable.text, cid)
	...
	elseif msgcontains(msg, "yes") and talkState[talkUser] == 3 then
		if getPlayerStorageValue(cid, Stg[cid]) < 1 then
			selfSay({"Very nice! Come back if you have found what's going on in this cult."}, cid)
			setPlayerStorageValue(cid, Stg[cid], 1)
		...
		elseif getPlayerStorageValue(cid, Stg[cid]) > 0 then
			selfSay({"You have not finished your work yet. Come back when you're done."}, cid)
			talkState[talkUser] = 2
			playerTopic[cid]= 2
		end
	...
	selfSay({"What a pitty! You can come back, when ever you want, if you have changed your opinion."}, cid)
	talkState[talkUser] = 0
