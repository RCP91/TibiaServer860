...
local function creatureSayCallback(cid, type, msg)
	...
	local Qline = getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline)
	

	if msgcontains(msg, 'join') then
		if Qline ~= -1 then
			selfSay('Wow, your stupidity would be pride and joy for every fool. You\'ve already applied as a member. Let\'s rather talk about your current mission.', cid)
			return true
		end
		selfSay('Do you wish to become a jester and join the fools guild?', cid)
	...
	elseif msgcontains(msg, 'mission') then		
		...
		if Qline == 1 then
			selfSay('Do you have a fitting flower for me?', cid)
			talkState[talkUser] = 3
		elseif Qline == 2 then			
			doPlayerAddItem(cid, 7478,1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission2, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 3)
			selfSay({'The next task will be a bit more difficult. I pride myself on being the major manufacturer of stink bombs in whole Tibia. Stop chuckling you fool aspirant ...',
		...
		elseif Qline == 3 then
			selfSay('Do you have acquired the exquisite stench we need?', cid)
			talkState[talkUser] = 3		
		elseif Qline == 4 then
			selfSay({'I think you\'re ready for a small promotion. Provided that you finish another mission, that is ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission3, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 5)
		elseif Qline == 5 then
			selfSay('So do you bring me a box full of pies?', cid)
			talkState[talkUser] = 3
		elseif Qline == 6 then
			selfSay({'Ah, the fool in training wants to see some action? Fine with me. I think you are ready for a big prank. But first I need 18 vials filled with wine ...',
				'Get them and report here about your mission.'}, cid)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission4, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 7)
		elseif Qline == 7 then
			selfSay('Do you have 18 vials of wine?', cid)
			talkState[talkUser] = 3
		elseif Qline == 8 then
			selfSay('Do you bring me Xodet\'s exchanged crate?', cid)
			talkState[talkUser] = 3
		elseif Qline == 9 then
			selfSay({'I have a fine prank in mind but without proper preparation, you\'d end up hacked in pieces by the dwarfs. By chance I found out about an artefact that might save the day ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission5, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.TriangleTowerDoor, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 10)
		elseif Qline == 10 then
			selfSay('Did you get the watch?', cid)
			talkState[talkUser] = 3
		elseif Qline == 11 then
			selfSay('Did you get the beard?', cid)
			talkState[talkUser] = 3
		elseif Qline == 12 then
			selfSay('I can\'t think of anything right now. Maybe you should try to get your jester outfit.', cid)
			talkState[talkUser] = 0
		elseif Qline == 13 then
			selfSay('You know, nothing has brought more fun and joy to humanity than the whoopee cushion. Its creation is a delicate matter though. Bring me 4 pieces of minotaur leather first and then lets talk about this mission.', cid)
			talkState[talkUser] = 0
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission6, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 14)
		elseif Qline == 14 then
			selfSay('Do you have the four minotaur leathers?', cid)
			talkState[talkUser] = 3
		elseif Qline == 15 then
			selfSay('Do you have the spider silk with you?', cid)
			talkState[talkUser] = 3
		elseif Qline == 16 then
			doPlayerAddItem(cid, 7485,1)
			selfSay({'Now that we have this whoopee cushion it would be a waste not to use it, don\'t you agree ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission6, 4)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 17)
			talkState[talkUser] = 0
		elseif Qline == 17 then	
			selfSay('Have you placed the whoopee cushion on the throne in Carlin?', cid)
			talkState[talkUser] = 3
		elseif Qline == 18 then	
			selfSay({'The time has come to play a prank on some arrogant Venorean. I found out that Carina, the jeweller, is terribly afraid of mice ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission7, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.QueenEloiseCatDoor, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 19)
			talkState[talkUser] = 0
		elseif Qline == 19 then	
			selfSay('Have you been to Venore and scared Carina as I asked you to?', cid)
			talkState[talkUser] = 3
		elseif Qline == 20 then	
			selfSay({'These Venoreans think they are safe from us. They couldn\'t be more wrong. Let them feel the wrath of the fools! Get a spoon and collect some sulphur from an inactive lava hole ...',
				'Be very careful when you get the easily inflammable substance. Then let\'s talk about the next part of your mission.'}, cid)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission8, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 21)
			talkState[talkUser] = 0
		elseif Qline == 21 then	
			selfSay('Have you collected the sulphur?', cid)
			talkState[talkUser] = 3
		elseif Qline == 22 then	
			selfSay('Do you have the leaves with you?', cid)
			talkState[talkUser] = 3
		elseif Qline == 23 then	
			doPlayerAddItem(cid, 7499,1)
			selfSay({'Now look at this, someone has made exploding cigars out of the sulphur and the leaves! Coincidentally I have a great idea how wecould use them ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission8, 4)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 24)
		elseif Qline == 24 then	
			selfSay('Have you delivered the exploding cigar to Theodore Loveless?', cid)
			talkState[talkUser] = 3
		elseif Qline == 25 then	
			doPlayerAddItem(cid, 7478,1)
			selfSay({'Well, I think as a true fool you have to think big. So our next little prank will be on a grand scale. Of course, this will need some preparation ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission9, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 26)
			talkState[talkUser] = 0
		elseif Qline == 26 then
			selfSay('Have you gotten the blood that we need?', cid)
			talkState[talkUser] = 3
		elseif Qline == 27 then
			selfSay('Have you acquired the constrictor\'s ink?', cid)
			talkState[talkUser] = 3
		elseif Qline == 28 then
			doPlayerAddItem(cid, 7490,1)
			selfSay({'Blood of a stalker and ink of a quara are the main ingredients of the prop for our next prank. Mix it together to obtain some nice vanishing ink ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission9, 4)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 29)
			talkState[talkUser] = 0
		elseif Qline == 29 then
			selfSay('So have you fooled the old Sam yet?', cid)
			talkState[talkUser] = 3
		elseif Qline == 30 then
			selfSay({'This time I have no real mission for you, rather a small errand. I bet you had expected some dangerous and exhausting quest but since I like you so much, I make it veeeeery easy for you ...',
		...
		elseif Qline == 31 then
			selfSay('Are you done with your little delivery mission?', cid)
			talkState[talkUser] = 3
		elseif Qline == 32 then
			selfSay({'I\'m sure you\'re wondering how many tiring missions old Bozo has still in store for you! Don\'t worry! You\'re almost done, only one final mission left ...',
			...
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission11, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 33)
			talkState[talkUser] = 0
		elseif Qline == 33 then
			selfSay('Have you been able to get hold of 5 pieces of white cloth?', cid)
			talkState[talkUser] = 3
		elseif Qline == 34 then
			selfSay('Do you have a piece of old and worn cloth?', cid)
			talkState[talkUser] = 3
		elseif Qline == 35 then
			selfSay('Well, my diligent pupil, have you scared the caliph yet?', cid)
			talkState[talkUser] = 3
		elseif Qline == 36 then
			selfSay('You are a fully trained fool now who has developed to an accomplished jester. Your final mission is to bring fun and joy to the whole world.', cid)
			talkState[talkUser] = 0
		
		end --Fin de mission	
	elseif msgcontains(msg, 'jester outfit') then
		if Qline == 12 then
			local targetValue = jesterOutfit[getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.JesterOutfit)]
			if not targetValue then
				return true
			end

			selfSay(targetValue.text[1], cid)
			talkState[talkUser] = 4
			value[cid] = targetValue
		else
			selfSay('I\'m sure it suits you well.', cid)
		end
	...
	elseif msgcontains(msg, 'yes') then
		...
		elseif talkState[talkUser] == 2 then
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questlog, 1)
			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission1, 1)
			selfSay({
		...
		elseif talkState[talkUser] == 3 then
			if Qline == 1 then
				--Entregar Flor
				if not doPlayerRemoveItem(cid, 7476,1) then
					selfSay('You don\'t have a fitting flower! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission1, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 2)
				selfSay('Indeed. This flower will do the trick, so to say. Just ask me about another mission whenever you\'re ready.', cid)
				talkState[talkUser] = 0
			elseif Qline == 3 then	
				--Entregar Flask
				if not doPlayerRemoveItem(cid, 7477,1) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission2, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 4)
				selfSay('Ugh. You got some efficient stuff there. I have to admit you have a talent for such distasteful matters. Perhaps you feel ready for another mission. Just ask me about it.', cid)
				talkState[talkUser] = 0
			elseif Qline == 5 then	
				--Entregar BOX
				if not doPlayerRemoveItem(cid, 7484,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				--Pie
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.PieBoxTimer) > 0
						and getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.PieBoxTimer) < os.time() then
					selfSay('Eeeek! What have you done?? These pies are crawling with bugs! Those must be the infamous parcel bugs! Get some new pies at once you wannabe fool, and this time without any bugs!', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission3, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 6)
				selfSay({'Excellent. Poor Harsky, poor Stutch. They will mourn the day they dared to yawn during a performance of the magnificent Bozo ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 7 then		
				if not doPlayerRemoveItem(cid, 2006, 18, 15) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission4, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 8)
				doPlayerAddItem(cid, 7482,1)
				selfSay({'Excellent. Poor Harsky, poor Stutch. They will mourn the day they dared to yawn during a performance of the magnificent Bozo ...',
				'However, I grant you the title of \'Fool in Training\' for your efforts. Just don\'t let it go to your head and don\'t use this highly spiritual title to show off ...',
				'If you are interested in another mission, talk to me.'}, cid)
				talkState[talkUser] = 0
			elseif Qline == 8 then	
				if not doPlayerRemoveItem(cid, 7483,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission4, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 9)
				selfSay({'I see. I guess we can expect a few drunken mages soon! Am I mistaken or are you up for some more challenging missions? Just ask me about if you feel foolish enough.'}, cid)
				talkState[talkUser] = 0
			elseif Qline == 10 then	
				if getPlayerItemCount(cid, 8187) == 0 then
					selfSay('No you have not! Why do only fools apply to the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission5, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 11)
				selfSay({'Oh boy, you will have so much fun with that watch if they don\'t catch and kill you. I envy you so much ... Bozo: So listen my little fool, this watch has the power to advance you in time. It will come in handy when you are stealing the beard of the dwarven emperor ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 11 then	
				if not doPlayerRemoveItem(cid, 7479,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end	
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission5, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 12)
				selfSay({'You got the beard and survived. I think for the first time in my whole life I\'m impressed, no wait, it was just a stone in my shoe ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 14 then	
				if not doPlayerRemoveItem(cid, 5878, 4) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end	
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission6, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 15)
				selfSay('Good, this was the first part. Now it gets difficult. To sew it together, we need a very fine yarn, as fine as the silk of a giant spider. Bring me some giant spider silk and talk to me about your mission.', cid)
				talkState[talkUser] = 0
			elseif Qline == 15 then	
				if not doPlayerRemoveItem(cid, 5879,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end	
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission6, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 16)
				selfSay('Let\'s see, a stitch here, a stitch there. Hey Presto! A whoopee cushion! Are you ready for the fun part? Then talk to me about your next mission.', cid)
				Npc():getPosition():sendMagicEffect(CONST_ME_POFF)	
				talkState[talkUser] = 0
			elseif Qline == 17 then	
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.WhoopeeCushion) ~= 1 then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission6, 5)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 18)
				selfSay('This will cause quite a commotion in Carlin. I think we should play some pranks in many more places to spread humour all over the world. Talk to me about your next mission to learn more about it.', cid)
				talkState[talkUser] = 0
			elseif Qline == 19 then	
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.ScaredCarina) ~= 1 then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission7, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 20)
				selfSay('Excellent. The Venoreans have been safe from my pranks far too long. If you are ready for another mission, talk to me.', cid)
				talkState[talkUser] = 0
			elseif Qline == 21 then	
				if not doPlayerRemoveItem(cid, 8204,1) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission8, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 22)
				selfSay('So much for part one of my foolish master plan. Now travel to Tiquanda and use a kitchen knife to cut some leaves of the jungle dweller bush for me. Bring them with you when you report about your mission.', cid)
				talkState[talkUser] = 0
			elseif Qline == 22 then		
				if not doPlayerRemoveItem(cid, 8109,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission8, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 23)
				selfSay('So far so good. I will prepare some nasty surprise for your next mission. Talk to me about it whenever you feel ready.', cid)
				talkState[talkUser] = 0
			elseif Qline == 24 then	
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Cigar) ~= 1 then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission8, 5)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 25)
				selfSay('You\'re hell of a guy, I bet you enjoyed it. As you represent everything a fool stands for, I give you this fool\'s sceptre to spiceup your jester outfit. If you are interested, there are even more missions for a fool like yourself. Just ask about them.', cid)
				...
				talkState[talkUser] = 0
			elseif Qline == 26 then	
				if not doPlayerRemoveItem(cid, 7488,1) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission9, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 27)
				selfSay({'Congratulations! Now let\'s talk about the complicated part. We need the ink of a quara constrictor. Use this vial on a fresh corpse to get the ink ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 27 then	
				if not doPlayerRemoveItem(cid, 7489,1) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission9, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 28)
				selfSay('Excellent. As a true fool you have of course no clue what these ingredients are good for, but I will enlighten you. Just ask about your next mission and I\'ll tell you everything you need to know.', cid)
				talkState[talkUser] = 0
			elseif Qline == 29 then
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Contract) ~= 2 then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission9, 5)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 30)
				selfSay('What a splendid prank! If you are ready for more, ask me about the next mission.', cid)
				talkState[talkUser] = 0
			elseif Qline == 30 then	
				--Mision de galletas
				doPlayerAddItem(cid, 8111,10) 
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission10, 1)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 31)
				selfSay({'Great! That\'s the word of a true fool! Maybe you want to write the names down. Here we go: delivered a cookie to: ...',
			...
			elseif Qline == 31 then	
				if player:getCookiesDelivered() ~= 10 then
					selfSay('No, you aren\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission10, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 32)
				selfSay('Indeed, you\'re done. By the way, you look a little bit tired and dirty. However, if you still have some power left, ask me about the next mission.', cid)
				talkState[talkUser] = 0
			elseif Qline == 33 then	
				if not doPlayerRemoveItem(cid, 5909,5) then
					selfSay('No, you haven\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				doPlayerAddItem(cid, 7500) 
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission11, 2)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 34)
				selfSay({'Well done. Anyway, I reeeally hate to tell you but ... as white as it\'s now we have no use for it. Don\'t worry though. There is a way to give it an old and worn look ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 34 then	
				if not doPlayerRemoveItem(cid, 7501,1) then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				doPlayerAddItem(cid, 7502) 
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission11, 3)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 35)
				selfSay({'Fine, fine. Now let\'s do the last step of our foolish plan. Smart as I am, I used a pair of scissors to create some old and worn bands from the cloth you have brought me. DON\'T touch it yet ...',
				...
				talkState[talkUser] = 0
			elseif Qline == 35 then		
				if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.ScaredKazzan) ~= 1 then
					selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
					talkState[talkUser] = 0
					return true
				end
				doPlayerAddItem(cid, 7957) 
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission11, 4)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 36)
				selfSay('By Kurik, I knew you have it in you. You are a fool after my fancy. Take this jester hat, you deserve it. It will go nicely with your jester costume.', cid)
		...
		elseif talkState[talkUser] == 4 then
			...
			if not doPlayerRemoveItem(cid, targetValue.removeItemId, 1) then
				selfSay('No, you don\'t! Why do only fools apply for the fools guild?', cid)
				talkState[talkUser] = 0
				return true
			end
			doPlayerRemoveItem(cid, targetValue.removeItemId, 1)

			setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.JesterOutfit, targetValue.newValue)
			if targetValue.addOutfit then
				player:addOutfit(270)
				player:addOutfit(273)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 13)
			end
			selfSay(targetValue.text[2], cid)
	...
	elseif msgcontains(msg, 'no') and talkState[talkUser] ~= 0 then
		if isInArray({1, 2}, talkState[talkUser]) then
			selfSay('Too bad, I\'m convinced you have it in you.', cid)
		elseif isInArray({3, 4}, talkState[talkUser]) then
			if getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline) == 11
					and getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.EmperorBeardShave) == 1 then
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline, 12)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Mission5, 3)
				selfSay({
			...
			elseif getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline) == 30 then
				selfSay('You won\'t be successful in the fool\'s world with such an attitude.', cid)
			elseif getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.Questline) == 35
					and getPlayerStorageValue(cid, Storage.WhatAFoolishQuest.LostDisguise) ~= 1 then
				doPlayerAddItem(cid, 7502, 1)
				setPlayerStorageValue(cid, Storage.WhatAFoolishQuest.LostDisguise, 1)
				selfSay('You wasted the disguise?? Why do only fools apply for the fools guild? Here... try again, but be wittier this time.', cid)
			else
				selfSay('Oh boy, why do only fools apply for the fools guild?', cid)
			end
		end
		talkState[talkUser] = 0
	end
