...
local player = Player(cid)
	if msgcontains(msg, "passage") or msgcontains(msg, "sail") then
		...
		elseif player:getStorageValue(Storage.SearoutesAroundYalahar.TownsCounter) >= 5 then
			npcHandler:say({
				"For the sake of profit, we established ship routes to {Ab\'Dendriel}, {Darashia}, {Venore}, {Ankrahmun}, {Port Hope}, {Thais}, {Liberty Bay}, {Carlin} or citys premium account {Gray Island}, {Issavi}, {krailos}, {Roshamuul} or {Oramond}?",
			}, cid)
			npcHandler.topic[cid] = 0
		else return false
		end
