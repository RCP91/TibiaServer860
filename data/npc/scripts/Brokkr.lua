...
function craftItem(cid, message, keywords, parameters, node)
    ...
    elseif (parameters.decline == true) then
        selfSay('Come back when you change your mind.', cid)
        npcHandler:resetNpc()
        return true
    end
    return false
...
local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
    ...
    if talkState[talkUser] == 1 then
        ...
        if doPlayerRemoveItem(cid,5944,getPlayerItemCount(cid,5944)) then
            ...
            selfSay("You don't have needed items!", cid) -- if orbs couldn't be removed
        end
        else
            selfSay("You don't have needed items!", cid) -- if backpack is empty
        end
        talkState[talkUser] = 0
        npcHandler:resetNpc()
        end
    ...
    if msgcontains(msg, "orb") or (msgcontains(msg, "infernal") and msgcontains(msg, "bolt")) then
        ...
        bolts = (getPlayerItemCount(cid,5944) * 2)
        if (getPlayerItemCount(cid,5944) > 0) then
        selfSay('Do you want to exchange ' .. orbs .. ' for ' .. bolts .. ' infernal bolts?', cid)
        talkState[talkUser] = 1
        else
        selfSay('If you bring me some soul orbs, you may exchange them for infernal bolts.', cid)
        end
    end
