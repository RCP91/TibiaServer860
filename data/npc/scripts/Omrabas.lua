 local keywordHandler = KeywordHandler:new()
local npcHandler = NpcHandler:new(keywordHandler)
NpcSystem.parseParameters(npcHandler)
local talkState = {}

function onCreatureAppear(cid)			npcHandler:onCreatureAppear(cid)			end
function onCreatureDisappear(cid)		npcHandler:onCreatureDisappear(cid)			end
function onCreatureSay(cid, type, msg)		npcHandler:onCreatureSay(cid, type, msg)		end
function onThink()		npcHandler:onThink()		end

local voices = {
	{ text = 'Typical. Easy come, easy go.' },
	{ text = 'He will PAY for this... They will ALL pay!' },
	{ text = '<groan>' },
	{ text = 'If I ever lay hands on him again...' }
}

--npcHandler:addModule(VoiceModule:new(voices))

local function creatureSayCallback(cid, type, msg)
	if not npcHandler:isFocused(cid) then
		return false
	end
	
	if msgcontains(msg, "mission") then
		if getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.QuestStart) < 1 then
			selfSay("Hmm. You could be of assistance, I presume. I need several body parts. I will reward you adequately. Interested?", cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.QuestStart, 1)
			talkState[talkUser] = 1
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission01) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission02) < 1 then
			selfSay("Ah hello, young friend! Did you bring me two ghoul snacks as requested?", cid)
			talkState[talkUser] = 2
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission02) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission03) < 1 then
			selfSay("Ah, young friend, I found a solution! Find me two {demonic skeletal hands}. That should do it. Now run along! Ask me for {mission} when you're done.", cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission03, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission03) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission04) < 1 then
			selfSay("Ah hello again! You look as if you could, er, lend me a hand or two? Yes?", cid)
			talkState[talkUser] = 3
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission04) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission05) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission05, 1)
			selfSay({
				"I need my heart back. I know where they have hidden it. Too afraid to destroy my beating heart, hah! ...",
				"It is in a dusty amphora in a sealed mass grave in the downmost cellar of my ancient home. The ruins lie to the north at the beach. ...",
				"Crash the amphoras to find my heart, and bring it to me."
			}, cid)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission06) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission07) < 1 then
			selfSay({
				"Yes? You have it? You what? Not in the amphoras? You picked it off someone else?!? ...",
				"Well, the important thing is that you have it. Let's see if it's still in good shape. {Give} it to me."
			}, cid)
			talkState[talkUser] = 4
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission07) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission08) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission08, 1)
			selfSay({
				"I will need brains - don't laugh! Ahem. I will need a stimulated brain, to be precise. ...",
				"Use two half-eaten brains with the Brain Heater Machine in the Necromancer halls and bring me the fused, stimulated brain. Now go!"
			}, cid)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission09) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission10) < 1 then
			selfSay("Yes, yes, hello. Tell me if you lost something. If not, do you have that stimulated brain with you?", cid)
			talkState[talkUser] = 5
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission10) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission11) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission11, 1)
			doPlayerAddItem(cid, 21402,1)
			selfSay({
				"Now that you have shown you've got the brains, I need you to show initiative. ...",
				"I will need something that can be adequately used as intestines. Something alive. Stuff it into this storage flask and return it to me!"
			}, cid)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission12) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission13) < 1 then
			selfSay("Hello, hello. Let's come to the point - did you find me some intestines?", cid)
			talkState[talkUser] = 6
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission13) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission14) < 1 then
			selfSay({
				"Good. As you may have gathered, the body parts you brought need to be assembled to form a whole body. ...",
				"This will happen through a necromantic ritual that we - or rather, you - now have to prepare. ...",
				"In a first step, it will involve retrieving specific artefacts of dark magic and hallowing the altars of the dark powers with them. ...",
				"After that, you will have to speak an ancient incantation to animate the body and fuse my soul within. ...",
				"But first things first. Are you ready to {undertake} the hallowing of the five altars?"
			}, cid)
			talkState[talkUser] = 7
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission15) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission16) < 1 then
			selfSay("Ah hello! Well done there, I felt the old powers settling down. Now, {ready} to hallow the next altar?", cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission16, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission16) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17) < 1 then
			selfSay({
				"I expected no less. Next is the Bonemarrow Altar, where the Dark Lord feasts upon the hallowed bones of...err. ...",
				"Ahem. Just take a yellowed bone or big bone, and hallow it - you don't know how? ...",
				"Huh. To HALLOW a yellowed bone, use it with one of the hallowed bonepiles in the Gardens Of Night. There are always bones around there - or were, in my days. ...",
				"Then, place the hallowed bone on the firebasin of the Bonemarrow Altar so the Dark Lord can consume it and grants us his power. Return to me after that."
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission18) < 1 then
			selfSay({
				"Are you still here? Come on, let's not laze about, go to the Gardens of Night - where? ...",
				"East of the Necromancer Halls, you can't miss it, just look for a lot of dark and white sand, dried trees, and priestesses! ...",
				"Or got a {problem} with finding bones? Hah!"
			}, cid)
			talkState[talkUser] = 8
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission20) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission21) < 1 then
			selfSay({
				"Ah, welcome, welcome! I felt that one! The Dark Lord is pleased with the gift you brought, so now we can proceed with everything as planned. ...",
				"If it had gone wrong though, he would have had your guts for gart... er... well, here you are, so - ready to get some {blood} flowing?"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission21, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission21) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission22) < 1 then
			selfSay({
				"Yes, I said blood. Important ingredient in necromantic rituals, usually. ...",
				"You need to spill some vials of blood tincture for this task. Probably means killing blood priests to get those vials. Ready to do this?"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission22, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission25) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission26) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission26, 1)
			selfSay({
				"Ah, it is invigorating to fell the dark flows, rushing through Drefia, once again! Capital. This has earned you a reward. ...",
				"Now, the next mission awaits, {yes}?"
			}, cid)
			talkState[talkUser] = 9
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission29) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission30) < 1 then
			selfSay({
				"Ah, finally. Exceptional! Don't you feel the earth awakening to our call? No? Oh. ...",
				"Anyway, you succeeded in hallowing the Fireglass Altar! Only one altar remains to be hallowed! Shall we {proceed}?"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission30, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission33) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission34) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission34, 1)
			selfSay({
				"Yes! YES! I have felt that! The altars are alive again! Well done! ...",
				"We are close now. Only one important thing remains: the incantation itself. We need the {scroll} for that."
			}, cid)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission42) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43) < 1 then
			selfSay('Yes? Do you have the {scroll} piece? ', cid)
			talkState[talkUser] = 11
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44) < 1 then
			selfSay({
				"Hah, developed a taste for it, have you? I believe the next scroll piece was hidden somewhere, my old friend being of a somewhat distrustful nature. ...",
				"The scent may not be lost, though - ask a shadow pupil if he can help - but be careful. ...",
				"Those shadow pupils are - strange. Try to find one who will answer and not kill you!"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission50) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51) < 1 then
			selfSay("Ah, hello! I take it you have the next scroll piece for me, {yes}?", cid)
			talkState[talkUser] = 12
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52) < 1 then
			selfSay({
				"Good! As you can imagine, I had a scroll piece, too. I hid it in my old quarters, northwest of the library. ...",
				"The door is magically sealed. Use this copper key with it to get inside. ...",
				"Beneath one of the chests is a secret stash, under a loose stone tile. The scroll piece should be inside. Off you go."
			}, cid)
			doPlayerAddItem(cid, 21489, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission57) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) < 1 then
			selfSay("Hello - what? You have the {scroll} piece, you say?", cid)
			talkState[talkUser] = 13
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59) < 1 then
			doPlayerAddItem(cid, 21464, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59, 1)
			selfSay({
				"Hahah, eager for it, I like that! ...",
				"One piece stayed in the hands of a beautiful priestess. ...",
				"As they never throw anything away that looks like an incantation - you get my drift. Find out where they still keep it! ...",
				"Oh, one more thing! They only talk to you when you look like a fellow summoner. It's dangerous to go without a cape. Use this."
			}, cid)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission64) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission65) < 1 then
			selfSay("Hello, young apprentice. Do you have that {scroll} piece from the priestess?", cid)
			doPlayerAddItem(cid, 21464, 1)
			talkState[talkUser] = 14
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission65) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission66) < 1 then
			selfSay({
				"That's the spirit! Speaking of which, you will have to look for a White Shade ...",
				"That's a ghost, in case you don't know. Goes by name of... Zarifan, if I recall correctly. ...",
				"His grave is somewhere to the south, somewhere deeper. And he only reacts to magic words. ...",
				"Err... it's embarrassing... the magic words are <mumble> <mumble> <ahem> ...",
				"'Friendship lives forever.' Silly, really. Untrue, as well. ...",
				"Anyway, say those three words - don't make me repeat them - and the old softie will tell you where the scroll is. Pathetic."
			}, cid)
			doPlayerAddItem(cid, 21464, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission66, 1)
		--elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission68) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69) < 1 then
			--selfSay("Welcome, welcome! Finally! The last scroll piece.... you do have it, haven't you?", cid)
			--talkState[talkUser] = 15
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission70) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission71) < 1 then
			selfSay("Don't keep me waiting. The last scroll piece - were you able to {restore} it?", cid)
			talkState[talkUser] = 16
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission73) == 1 then--and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission74) < 1 then
			selfSay({
				"It failed! IT FAILED! WHY? What have you done! This must be your fault! ...",
				"You... did... that was a recipe for chicken soup! No wonder the scroll failed! Now... all is lost ...",
				"<sobs drily> I never want to see you again! You and your dirty gravedigger hands! Take this, you ungrateful, useless, imbecile... human! ...",
				"Now... get out! And never, ever, dare come back! You ruined EVERYTHING!"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission73, 2)
			doPlayerAddItem(cid, 21452, 1)
		end
	elseif msgcontains(msg, "yes") then
		if talkState[talkUser] == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.QuestStart) == 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission01, 1)
			selfSay({
				"Very sensible of you. I will pay you handsomely for your help. ...",
				"All you have to do is fetch diverse fresh body parts and then prepare the resurrection ritual. Nothing out of the ordinary. ...",
				"First, I need two arms. Ghouls usually carry some as a snack. Two ghoul snacks should not be hard to get! Return when you have them."
			}, cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 2 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission01) == 1 then
			if doPlayerRemoveItem(cid, 12423, 2) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission02, 1)
				selfSay("Splendid! What? They're half gnawed! There are no hands! Hrmmm. Let me think of a solution. Ask me for a new {mission}.", cid)
				talkState[talkUser] = 0
			else
				selfSay("You don't have two ghoul snacks.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 3 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission03) == 1 then
			if doPlayerRemoveItem(cid, 10564, 2) then
				selfSay("Yes. Those will be adequate. Talk to me again if you want to continue with your next {mission}.", cid)
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission04, 1)
				talkState[talkUser] = 0
			else
				selfSay("You don't have two demonic skeletal hands.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 4 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission06) == 1 then
			if doPlayerRemoveItem(cid, 21394, 1) then
				selfSay("Ah... <sighs> Very good. Just say the word when you are ready for the next {mission}.", cid)
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission07, 1)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my heart.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 5 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission09) == 1 then
			if doPlayerRemoveItem(cid, 21395, 1) then
				selfSay("Ah... <sighs> Very good. Just say the word when you are ready for the next {mission}.", cid)
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission10, 1)
				talkState[talkUser] = 0
			else
				selfSay("You don't have the brain.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 6 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission12) == 1 then
			if doPlayerRemoveItem(cid, 21403, 1) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission13, 1)
				selfSay({
					"Ah... interesting. A snake? Not bad, not bad at all. ...",
					"Right. Now, we need to waken the old powers through ritual. Let me know when you are ready for this {mission}."
				}, cid)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my intestine.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission22) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission23) < 1 then
			selfSay({
				"Ah, I knew you were a sturdy fellow! Necromancer material if ever I saw one! ...",
				"Well then, the next task is to anoint the Bloodgong Altar. This will animate the dark flows we need for the ritual. ...",
				"You need to spill a vial of blood tincture on each of the four sacrifical stones of the altar in order to anoint them. ...",
				"Then, toll the Bloodgong north of the sacrificial stones to set the dark flows going. Return to me after that."
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission23, 1)
		elseif talkState[talkUser] == 9 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission26) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission27) < 1 then
			selfSay({
				"Excellent. To bind the earthly powers, we, I mean you, must worship at the Fireglass Altar. ...",
				"This means scattering sacred ashes from the Ember Chamber on the sacrificial stones of the Fireglass Altar. ...",
				"To gather the sacred ashes, you have to use magic chalk with the Shadow Fire in the Ember Chamber. ...",
				"Gather the resulting ashes from the Shadow Hearth, and scatter them on the Fireglass Altar's sacrificial stones. Then return here."
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission27, 1)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 10 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission34) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission35) < 1 then
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission35, 1)
			selfSay({
				"Superb! You won't regret this. I will reward you beyond your wildest dreams! ...",
				"I know that one of my former friends joined the blood priests later on. He made it quite high before he was ritually killed. ...",
				"Go find a blood priest you can talk to. Ask him, but subtly. And never tell anyone what the scroll does!"
			}, cid)
			talkState[talkUser] = 0
		elseif talkState[talkUser] == 11 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission42) == 1 then
			if doPlayerRemoveItem(cid, 21250, 1) then
				selfSay("This is it! This is it! Well done, well done! And now, on to the {next} scroll piece, {yes}?", cid)
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43, 1)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44) < 1 then
			selfSay({
				"Hah, developed a taste for it, have you? I believe the next scroll piece was hidden somewhere, my old friend being of a somewhat distrustful nature. ...",
				"The scent may not be lost, though - ask a shadow pupil if he can help - but be careful. ...",
				"Those shadow pupils are - strange. Try to find one who will answer and not kill you!"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44, 1)
		elseif talkState[talkUser] == 12 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission50) == 1 then
			if doPlayerRemoveItem(cid, 21250, 1) then
				selfSay("Indeed it is! The second scroll piece! Splendid! Here you go - for your trouble. And now, on to the {next} scroll piece, {yes}? ", cid)
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51, 1)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52) < 1 then
			selfSay({
				"Good! As you can imagine, I had a scroll piece, too. I hid it in my old quarters, northwest of the library. ...",
				"The door is magically sealed. Use this copper key with it to get inside. ...",
				"Beneath one of the chests is a secret stash, under a loose stone tile. The scroll piece should be inside. Off you go."
			}, cid)
			doPlayerAddItem(cid, 21489, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52, 1)
		elseif talkState[talkUser] == 13 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission57) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) < 1 then
			if doPlayerRemoveItem(cid, 21250, 1) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58, 1)
				selfSay({
					"Oh, praise the Dark Lord! It is my scroll piece! Give it here! You can have this instead. ...",
					"Ahh, that feels GOOD. Now, only two pieces left to hunt down! On to the {next} scroll piece, yes?"
				}, cid)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59) < 1 then
			doPlayerAddItem(cid, 21464, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59, 1)
			selfSay({
				"Hahah, eager for it, I like that! ...", "One piece stayed in the hands of a beautiful priestess. ...",
				"As they never throw anything away that looks like an incantation - you get my drift. Find out where they still keep it! ...",
				"Oh, one more thing! They only talk to you when you look like a fellow summoner. It's dangerous to go without a cape. Use this."
			}, cid)
		elseif talkState[talkUser] == 15 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission68) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69) < 1 then

			if doPlayerRemoveItem(cid, 21250, 1) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69, 1)
				selfSay({
					"Ahhh, capital, capital. Good girl for keeping it for me. I'll take back my cape now, thank you. ...",
					"Now - the final part of the scroll! Ready to go retrieve it?"
				}, cid)
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission70) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission71) < 1 then
			selfSay({
				"That's the spirit! Speaking of which, you will have to look for a White Shade ...",
				"That's a ghost, in case you don't know. Goes by name of... Zarifan, if I recall correctly. ...",
				"His grave is somewhere to the south, somewhere deeper. And he only reacts to magic words. ...",
				"Err... it's embarrassing... the magic words are <mumble> <mumble> <ahem> ...",
				"'Friendship lives forever.' Silly, really. Untrue, as well. ...",
				"Anyway, say those three words - don't make me repeat them - and the old softie will tell you where the scroll is. Pathetic."
			}, cid)
			doPlayerAddItem(cid, 21464, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission71, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69, 1)
		elseif talkState[talkUser] == 15 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission72) < 1 then
			if doPlayerRemoveItem(cid, 21250, 1) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission72, 1)
				selfSay({
					"I am so excited! Finally, we - wait. What is this? That... is not the complete scroll piece. ...",
					"I don't care what you say! This is a disaster! We need a complete scr - wait. I have an idea. ...",
					"There must be a copy or something in my old library. something to use with the scroll, to find the missing words. ...",
					"There must be. Go look there. Next to the fiveserrated room, a small library. Go go go!"
				}, cid)
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 17 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission73) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission74) < 1 then
			selfSay({
				"Then let's go. Take my skull and the incantation scroll to the working station in the fiveserrated room ...",
				"Where the lava flows and the southern legs of the room meet. Place the Skull on the sacrificial stone and use the scroll ...",
				"AND I shall be made whole, and YOU shall be RICH!"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission74, 1)
			doPlayerAddItem(cid, 21251, 1)
			doPlayerAddItem(cid, 21476, 1)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, 'demonic skeletal hands') or msgcontains(msg, 'demonic skeletal hand') then
		selfSay("What? Hack some off from a demon skeleton, of course! Now get moving.", cid)
	elseif msgcontains(msg, 'give') and talkState[talkUser] == 4 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission06) == 1 then
		if doPlayerRemoveItem(cid, 21394, 1) then
			selfSay("Ah... <sighs> Very good. Just say the word when you are ready for the next {mission}.", cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission07, 1)
			talkState[talkUser] = 0
		else
			selfSay("You don't have my heart.", cid)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, 'undertake') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission13) == 1 and talkState[talkUser] == 7  then
		selfSay({
			"Good! Let's not waste time. The first altar you must hallow is the Dragonsoul Altar, at the eastern side of the room. ...",
			"To hallow it, dig out three dragon tears and place them on the altar to appease the dark powers. ...",
			"What? Oh, you'll find some dragon tears near dragon skulls... where? ...",
			"In the Dragonbone Cemetery, OBVIOUSLY! Must I explain EVERYTHING to you? Now go!"
		}, cid)
		setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission14, 1)
		doPlayerAddItem(cid, 21401,3)
		talkState[talkUser] = 0
	elseif msgcontains(msg, 'ready') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission16) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17) < 1 then
		selfSay({
			"I expected no less. Next is the Bonemarrow Altar, where the Dark Lord feasts upon the hallowed bones of...err. ...",
			"Ahem. Just take a yellowed bone or big bone, and hallow it - you don't know how? ...",
			"Huh. To HALLOW a yellowed bone, use it with one of the hallowed bonepiles in the Gardens Of Night. There are always bones around there - or were, in my days. ...",
			"Then, place the hallowed bone on the firebasin of the Bonemarrow Altar so the Dark Lord can consume it and grants us his power. Return to me after that."
		}, cid)
		setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17, 1)
	elseif msgcontains(msg, 'problem') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission17) == 1 and talkState[talkUser] == 8 then
		selfSay("What? No bones around you say? Hrmmm. Wait. Check the skull heap here - that's right - hah! There! Now get to work!", cid)
		setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission18, 1)
		doPlayerAddItem(cid, 21407, 3)
		talkState[talkUser] = 0
	elseif msgcontains(msg, 'blood') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission21) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission22) < 1 then
		selfSay({
			"Yes, I said blood. Important ingredient in necromantic rituals, usually. ...",
			"You need to spill some vials of blood tincture for this task. Probably means killing blood priests to get those vials. Ready to do this?"
		}, cid)
		setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission22, 1)
	elseif msgcontains(msg, 'proceed') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission30) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission31) < 1 then
		setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission31, 1)
		doPlayerAddItem(cid, 21448, 1)
		doPlayerAddItem(cid, 21482, 1)
		selfSay({
			"<reverential> The final altar that remains to be hallowed... the Shadowthrone. ...",
			"Only a candle made of human tallow placed before each shadow statue in the right order, will awaken it. ...",
			"I say, are you sick? Human tallow candles are a perfectly good tool - don't look at me like that! Be professional about this, will you? ...",
			"I have some candles hidden deep down in the lich caves, east of here. Here's the key that opens the trapdoor to the cache. ...",
			"And take this parchment. Identify the right candles with it, take three with you and place one in front of the three shadowthrone statues, starting counterclockwise. Now go!"
		}, cid)
	elseif msgcontains(msg, 'scroll') then
		if getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission34) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission35) < 1 then
			selfSay({
				"Well, it is a rather long story. The short version: I had friends - no need to snigger. ...",
				"We were five highly brilliant dark summoners. We joined our forces to find the scroll of Youth and Life Eternal. ...",
				"But when we finally found it after years of toiling and danger, we quarreled over who could have it. The scroll tore. ...",
				"We each retained one piece of the scroll. Well, that was a long time ago. ...",
				"But I think I have a clue where we can find the first piece of the scroll. Would you go looking?"
			}, cid)
			talkState[talkUser] = 10
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission42) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43) < 1 then
			selfSay("Yes? Do you have the {scroll} piece?", cid)
			talkState[talkUser] = 11
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission50) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51) < 1 then
			selfSay("Ah, hello! I take it you have the next {scroll} piece for me, {yes}?", cid)
			talkState[talkUser] = 12
		elseif talkState[talkUser] == 13 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission57) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) < 1 then
			if doPlayerRemoveItem(cid, 21250, 1) then
				setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58, 1)
				selfSay({
					"Oh, praise the Dark Lord! It is my scroll piece! Give it here! You can have this instead. ...",
					"Ahh, that feels GOOD. Now, only two pieces left to hunt down! On to the {next} scroll piece, yes?"
				}, cid)
				talkState[talkUser] = 0
			else
				selfSay("You don't have my scroll.", cid)
				talkState[talkUser] = 0
			end
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission64) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission65) < 1 then
			selfSay("Hello, young apprentice. Do you have that {scroll} piece from the priestess?", cid)
			doPlayerAddItem(cid, 21464, 1)

		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission68) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69) < 1 then
			selfSay("Welcome, welcome! Finally! The last {scroll} piece.... you do have it, haven't you?", cid)
			talkState[talkUser] = 15
		end
	elseif msgcontains(msg, 'next') then
		if getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission43) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44) < 1 then
			selfSay({
				"Hah, developed a taste for it, have you? I believe the next scroll piece was hidden somewhere, my old friend being of a somewhat distrustful nature. ...",
				"The scent may not be lost, though - ask a shadow pupil if he can help - but be careful. ...",
				"Those shadow pupils are - strange. Try to find one who will answer and not kill you!"
			}, cid)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission44, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission51) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52) < 1 then
			selfSay({
				"Good! As you can imagine, I had a scroll piece, too. I hid it in my old quarters, northwest of the library. ...",
				"The door is magically sealed. Use this copper key with it to get inside. ...",
				"Beneath one of the chests is a secret stash, under a loose stone tile. The scroll piece should be inside. Off you go."
			}, cid)
			doPlayerAddItem(cid, 21489, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission52, 1)
		elseif getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission58) == 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59) < 1 then
			doPlayerAddItem(cid, 21464, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission59, 1)
			selfSay({
				"Hahah, eager for it, I like that! ...",
				"One piece stayed in the hands of a beautiful priestess. ...",
				"As they never throw anything away that looks like an incantation - you get my drift. Find out where they still keep it! ...",
				"Oh, one more thing! They only talk to you when you look like a fellow summoner. It's dangerous to go without a cape. Use this."
			}, cid)
		end
	elseif msgcontains(msg, 'restore') and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission69) <= 1 and getPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission73) < 1 then --and talkState[talkUser] == 15 then
		if doPlayerRemoveItem(cid, 21474, 1) then
			selfSay({
				"I knew it! I knew I had made a copy! Oh, I am so clever! ...",
				"Now, watch this. <mumbles darkly> THERE. THE COMPLETE INCANTATION SCROLL. ...",
				"You have done well. Only one thing remains, and you shall be rich beyond your dreams. Ready for the really final task?"
			}, cid)
			--setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission72, 1)
			setPlayerStorageValue(cid, Storage.GravediggerOfDrefia.Mission73, 1)
			talkState[talkUser] = 17


		else
			selfSay("You don't have my scroll.", cid)
			talkState[talkUser] = 0
		end
		else selfSay("Chzzzz. wtf??@! leave.", cid)
	end
	return true
end

npcHandler:setCallback(CALLBACK_MESSAGE_DEFAULT, creatureSayCallback)
npcHandler:setMessage(MESSAGE_GREET, "At last, a visitor! Welcome to my... humble abode.")
npcHandler:addModule(FocusModule:new())
