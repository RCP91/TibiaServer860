...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission") or msgcontains(msg, "task")) then
		if(getPlayerStorageValue(cid, 41600) < 1) and getPlayerStorageValue(cid, 43600) <= os.time() then
			selfSay("Oh, you want some work? You can help us, alright. Did you know that the people of the city think those rabbit feet are actually lucky charms?", cid)
			talkState[talkUser] = 1
		elseif(getPlayerStorageValue(cid, 41600) == 1) then
			selfSay("Good to see you back. Now, did you bring us the lucky charms?", cid)
			talkState[talkUser] = 3


			elseif getPlayerStorageValue(cid, 43600) > os.time() then
			selfSay("You need wait some hours to take other mission again or you are still on a mission.", cid)
	...
	------------------------ FINISH MISSION 01 ------------------------
		elseif(getPlayerStorageValue(cid, 41600) == 2) then
			...
			selfSay("So what do you say, in for this one?", cid)
			talkState[talkUser] = 4
		elseif(getPlayerStorageValue(cid, 41600) == 3) and getPlayerStorageValue(cid, 41500) > 4 then
			selfSay("Ah there you are. So, did you repair all the broken structures?", cid)
	...
	------------------------ FINISH MISSION 02 ------------------------
		elseif(getPlayerStorageValue(cid, 41600) == 4) then
			...
			selfSay("They will enter the forest near our camp which is where you come in - uhm I mean you do want to help us with this, right?", cid)
			talkState[talkUser] = 6
		elseif(getPlayerStorageValue(cid, 41600) == 5) and getPlayerStorageValue(cid, 41660) == 4 then
			selfSay("You did it!! And I assume you took only what you needed? Heh. No, I know it. Because my men took the rest. Thanks for helping us, you did a very good job. In fact I have a little 'extra' for you here, thanks again.", cid)
				------------------ RESET STORAGE --------------------
	...
	elseif(msgcontains(msg, "yes")) then
		if(talkState[talkUser] == 1) then
			selfSay("Good, because that is exactly what you will help us with - getting more 'lucky charms'. If we won't get our hands on new charms in time, we will surely have to starve... during the autumn. That would be a hard time for all of us. So... you in?", cid)
		...
		elseif(talkState[talkUser] == 2) then
			...
			selfSay("If you need some more ribbons just come back to me and ask.", cid)
			setPlayerStorageValue(cid, 41600, 1)
			setPlayerStorageValue(cid, 42600, 1) -- quest log
			doPlayerAddItem(cid, 13158, 7)
			talkState[talkUser] = 0
		elseif(talkState[talkUser] == 3) and getPlayerItemCount(cid, 13160) >= 7 then
			selfSay("Good hunt. That will be enough to help us uhm... get through the winter yes. Now if you want to help us getting even more lucky charms, you can always ask.", cid)
			doPlayerRemoveItem(cid, 13160, 7)
			setPlayerStorageValue(cid, 41600, 2)
			setPlayerStorageValue(cid, 42600, 2) -- quest log
			talkState[talkUser] = 0

			elseif(talkState[talkUser] == 3) and getPlayerItemCount(cid, 13160) <= 6 then
			selfSay("You do not have sufficient rabbit's foot.", cid)

		------------------------ FINISH MISSION 01 ------------------------
		elseif(talkState[talkUser] == 4) then
			selfSay("Good, good. Do you remember the old saying? If it ain't broken, it was not made by us. Now, off you go!", cid)
		...
		elseif(talkState[talkUser] == 5) then
			...
			selfSay("The walls look as if they will come off in a matter of hours. Oh well, you can always come back and repair this mess, ask me for a task if you want to. Yeah, yeah and here's your reward for today.", cid)
		...
		elseif(talkState[talkUser] == 6) then
			...
			selfSay("So... it's entirely up to you, will you do this or not?", cid)
		...
		elseif(talkState[talkUser] == 7) then
			...
			selfSay("Off you go and - good hunt, heh.", cid)
