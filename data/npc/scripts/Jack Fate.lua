...
local function addTravelKeyword(keyword, cost, destination, text, condition)
	...
	if keyword == 'goroma' then
		keywordHandler:addKeyword({keyword}, StdModule.say, {npcHandler = npcHandler, text = 'Never heard about a place like this.'}, function(player) return getPlayerStorageValue(cid, Storage.TheShatteredIsles.AccessToGoroma) ~= 1 end)
	end
...
addTravelKeyword('yalahar', 275, Position(32816,31272, 6), nil, function(player) return getPlayerStorageValue(cid, Storage.SearoutesAroundYalahar.LibertyBay) ~= 1 and getPlayerStorageValue(cid, Storage.SearoutesAroundYalahar.TownsCounter) < 5 end)
