...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 10029 then
		...
		if Game.getStorageValue(GlobalStorage.FerumbrasAscendantQuest.FlowerPuzzleTimer) >= 1 then
			...
			Player.setStorageValue(GlobalStorage.FerumbrasAscendantQuest.FlowerPuzzleTimer, 1)
			addEvent(Player.setStorageValue, 30 * 1000, GlobalStorage.FerumbrasAscendantQuest.FlowerPuzzleTimer, 1)
