...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
	...
	if tile and tile:getItemCount() > 30 then
		...
		return false
	 	...
	 	-- WARR ANTIENTROSA> 20 items
	if item:getId() == 2197 then
	if self:getStorageValue(STORAGE_PLAYER_IN_ARENA) == 1 then
	    if toPosition.y == CONST_SLOT_NECKLACE then
	        if self:getStorageValue(9816) >= os.time() then
	           	self:sendCancelMessage("Sorry, not possible.")
	        	return false
	        else
	            self:setStorageValue(9816, os.time() + 5)
	        end
	    end
	end
end
